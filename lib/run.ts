...
export async function getRun(): Promise<Run> {
  ...
  cached = {
    ...
    honestMoment: {
      assumption:
        "Бриф исходил из того, что в чаты маленьких команд конкуренты не лезут — это и была заявленная «незанятая точка».",
      discovery:
        "Ресёрч нашёл Ducalis (а также Featurebase, Savio, Olvy), которые уже стягивают фидбэк из Telegram / Slack / Discord прямо в роадмап.",
      correction:
        "Позиционирование скорректировано: дифференциация сместилась с «мы единственные в чатах» на «Telegram-first + полный цикл до роадмапа для малых команд».",
